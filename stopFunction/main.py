...
# Définition de la fonction pour arrêter une instance Google Cloud
def stop_instance(request):
    try:
        ...
        # Arrêt de l'instance en utilisant le client de l'API Google Cloud Compute
        client = compute_v1.InstancesClient()
        operation = client.stop(project=project, zone=zone, instance=instance)

        # Boucle pour attendre que l'opération d'arrêt soit terminée
        ...
        print("Instance arrêtée avec succès") # Message de débogage pour confirmer l'arrêt réussi

        return 'Instance arrêtée avec succès', 200 # Réponse HTTP de succès
